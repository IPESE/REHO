...
class reho(district_decomposition):
    ...
    def __find_obj1_lower_bound(self, Scn_ID):
        ...
        if self.method['district-scale']:
            ...
            if self.method['use_facades'] or self.method['use_pv_orientation']:
                REHO = compact_optimization(self.infrastructure, self.buildings_data, self.parameters, self.set_indexed, self.cluster, scenario, self.method, self.solver, self.qbuildings_data, self.csv_data)
            else:
                REHO = compact_optimization(self.infrastructure, self.buildings_data, self.parameters, self.set_indexed, self.cluster, scenario, self.method, self.solver, self.csv_data)
            ...
            if self.method['district-scale']:
                ...
                if self.method['use_facades'] or self.method['use_pv_orientation']:
                    REHO = compact_optimization(self.infrastructure, self.buildings_data, self.parameters, self.set_indexed,
                                                self.cluster, scenario, self.method, self.solver, self.qbuildings_data, self.csv_data)
                else:
                    REHO = compact_optimization(self.infrastructure, self.buildings_data, self.parameters, self.set_indexed,
                                                self.cluster, scenario, self.method, self.solver, self.csv_data)
                ampl, exitcode = REHO.solve_model()
                ...
                if self.method['district-scale']:
                    ...
                    if self.method['use_facades'] or self.method['use_pv_orientation']:
                        REHO = compact_optimization(self.infrastructure, self.buildings_data, self.parameters,
                                                    ...
                                                    scenario, self.method, self.solver, self.qbuildings_data, self.csv_data)
                    else:
                        REHO = compact_optimization(self.infrastructure, self.buildings_data, self.parameters,
                                                    self.set_indexed,
                                                    self.cluster,
                                                    scenario, self.method, self.solver, self.csv_data)
                    ampl, exitcode = REHO.solve_model()
    ...
    def fix_utilities(self, Pareto_ID, Scn_ID, cluster, df_Unit=None, scenario=None):
        ...
        if self.method['use_facades'] or self.method['use_pv_orientation']:
            REHO = compact_optimization(self.infrastructure, self.buildings_data, self.parameters, self.set_indexed,
                                        self.cluster, scenario_fix_uti, self.method, self.solver, self.qbuildings_data, self.csv_data)
        else:
            REHO = compact_optimization(self.infrastructure, self.buildings_data, self.parameters, self.set_indexed,
                                        self.cluster, scenario_fix_uti, self.method, self.solver, self.csv_data)
        ampl = REHO.build_model_without_solving()
    ...
    def single_optimization(self, Pareto_ID=0):
        ...
        if self.method['district-scale']:
            ...
            if self.method['use_facades'] or self.method['use_pv_orientation']:
                REHO = compact_optimization(self.infrastructure, self.buildings_data, self.parameters, self.set_indexed,
                                            self.cluster, self.scenario, self.method, self.solver, self.qbuildings_data, self.csv_data)
            else:
                REHO = compact_optimization(self.infrastructure, self.buildings_data, self.parameters, self.set_indexed,
                                            self.cluster, self.scenario, self.method, self.solver, self.csv_data)
            ampl = REHO.build_model_without_solving()
