...
def plot_economical_feedin_price(df_KPI, df_unit, total_surface_m2, bounds, roof_max_m2, SS, CN, plot_type, resolution):
...
def plot_rainbow(df_plot_last, feed_in_prices, No_feed_in, df_plot_inv_induced, bounds, save_fig):
    # plotting
    fig, ax = plt.subplots()
    for demand_price in df_plot_last.columns:
        if plot_type == "demand_feed_in_E_gen_pv":
            cs = ax.scatter(df_plot_last.index, np.repeat(demand_price, No_feed_in), c=df_plot_last[demand_price].values, s=5, cmap=cm, vmin=df_plot_last.min().min(), vmax=df_plot_last.max().max() )
...
df_plot_last = {}
df_plot_inv_induced = {}
list_districts = [277, 10559, 14824, 17316, 17335]
extrapolation = pd.DataFrame.from_dict({17335: 54.5, 277: 428.55, 10559: 281.57, 14824: 98.57, 17316: 207.82}, orient="Index") # ERA CH districts mio m2
...
for n in list_districts:
    ...
    with open('../../shared_executables_REHO/ECOS paper/2023/results/' + picklename, 'rb') as f:
    ...
    df_plot_last[n], feed_in_prices, No_feed_in, df_plot_inv_induced[n] = plot_economical_feedin_price(df_KPI, df_u,  total_surface_m2, bounds, roof_max_m2, m2_SS, m2_carbon_neutral, plot_type, resolution)
    print(n, " processed")
    for col in df_plot_last[n]:
        df_plot_last[n][col] = df_plot_last[n][col].replace(np.nan, 0)
        last_id = df_plot_last[n][col][df_plot_last[n][col] > 0].index[-1]
        index_max_PV = df_plot_last[n][col].loc[last_id:].index[1:]
        df_plot_last[n][col].loc[index_max_PV] = df_plot_last[n].max().max()


# aggregate data and extrapolate to CH
df_plot_last_CH = df_plot_last[list_districts[0]] * 0
for n in PV_area:
    df_plot_last_CH = df_plot_last_CH + df_plot_last[n] / PV_area[n] * extrapolation[0].loc[n] * 140 # 140 km2 roof area usable (EnergyScope)
df_plot_last_CH = sp.ndimage.filters.gaussian_filter(df_plot_last_CH, 1.5, mode='nearest')
...
# plot
plot_rainbow(df_plot_last_CH, feed_in_prices, No_feed_in, df_plot_inv_induced, bounds, save_fig=True)
