...
var AC_E_cooling{h in House,u in UnitsOfType['Air_Conditioner'] inter UnitsOfHouse[h],p in Period,t in Time[p],T in AC_Tsupply} >= 0,<= Units_Fmax[u]*AC_COP[h,u,p,t,T];

#-Cooling
subject to AC_energy_balance{h in House,u in UnitsOfType['Air_Conditioner'] inter UnitsOfHouse[h],st in StreamsOfUnit[u],p in Period,t in Time[p],T in AC_Tsupply: T = Streams_Tin[st,p,t]}:
sum{se in ServicesOfStream[st]} Streams_Q[se,st,p,t] = AC_COP[h,u,p,t,T]*AC_E_cooling[h,u,p,t,T];
...
Units_demand['Electricity',u,p,t] = sum{T in AC_Tsupply}(AC_E_cooling[h,u,p,t,T]);

#-Minimum PartLoad
subject to AC_c2{h in House,u in UnitsOfType['Air_Conditioner'] inter UnitsOfHouse[h],p in Period,t in Time[p]}:
sum{T in AC_Tsupply} (AC_E_cooling[h,u,p,t,T]/AC_Pmax[h,u,p,t,T]) <= Units_Mult[u]*AC_partload_max[u];

subject to AC_c3{h in House,u in {'Air_Conditioner_DHN_'&h},p in Period,t in Time[p]}:
Units_supply['Heat',u,p,t] = Units_demand['Electricity',u,p,t] + sum{st in StreamsOfUnit[u],T in AC_Tsupply: T = Streams_Tin[st,p,t]}AC_COP[h,u,p,t,T]*AC_E_cooling[h,u,p,t,T];
