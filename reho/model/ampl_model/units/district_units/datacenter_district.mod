...
param DC_heat_recovery{u in UnitsOfType['DataHeat']} default 1; #assumes heat recovery systems are installed in the data centers, so that all heat can be recovered, binary parameter
param PUE{u in UnitsOfType['DataHeat']} default 1.3; # Power Usage Effectiveness, ratio of total building energy usage to the energy used by the IT equipment alone, can vary between 1.1 and 2.0, but is typically around 1.3 for modern data centers
param utilisation_factor{u in UnitsOfType['DataHeat']} default 0.327; # Utilisation factor for data centers, represents the fraction of the total capacity that is actually used, typically around 0.3 to 0.4, 0.327 is taken from the RCP clusters at EPFL
