...
if __name__ == '__main__':
    ...
    for transformer in transf_list:
        ...
        try:
            ...
            if f'heating_system_2_{year}' in pathway_data.columns:
                parameters['ThermalSolar_install'] = np.array([[1] if qbuildings_data['buildings_data'][key][f'heating_system_2_{year}'] == 'ThermalSolar' else [0] for key in qbuildings_data['buildings_data'].keys()])
            ...
            if parameters['PV_install'].sum() > 0:
                parameters['PV_mult'] = np.array(
                    [[qbuildings_data['buildings_data'][key][f'pv_mult_{year}']]
                     if qbuildings_data['buildings_data'][key][f'pv_mult_{year}'] > 0
                     else [-1]
            ...
            reho = REHO(qbuildings_data=qbuildings_data, units=units, grids=grids, parameters=parameters,
                        cluster=cluster, scenario=scenario, method=method, solver="gurobiasl")
            ...
            # Save results
            reho.save_results(format=['pickle'], filename=f'{results_folder}/pathway_{year}_{transformer}')
    ...
    process_time_df.to_csv(str(Path(__file__).parent /'results'/f'{results_folder}/process_time_{year}.csv'), index=False)

    error_transf_df = pd.DataFrame({'Transformers': error_transf, 'Error': error_list})
    error_transf_df.to_csv(str(Path(__file__).parent /'results'/f'{results_folder}/error_transf_{year}.csv'), index=False)