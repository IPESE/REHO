...
def prepare_dfs(df_Economics, indexed_on='Scn_ID', neg=False, premium_version=None,
    ...
    if premium_version is not None:
        data_opex.loc[('avoided', 'solar_premium'), :] = data_opex.loc[('avoided', 'PV_SC')] * (
                premium_version[0] - premium_version[1]) / premium_version[0]
...
def plot_performance(results, plot='costs', indexed_on='Scn_ID', label='EN_long', add_annotation=True, per_m2=False,
                     additional_costs={}, scc=0.177, filename=None,
    ...
    additional_costs: dict
        Dictionary of additional costs to include (choose between 'isolation', 'mobility', and 'ict') and scaling values.
    scc: float
    ...
    if plot == 'costs':
        ...
        indexes, data_capex, data_opex = prepare_dfs(df_Economics, indexed_on, neg=True,
                                                     additional_costs=additional_costs,
                                                     scaling_factor=scaling_factor)
        data_opex = data_opex.drop("avoided", level='type')
        data_op = pd.DataFrame(0, columns=[indexes], index=data_opex.index)
        data_constr = pd.DataFrame(0, columns=[indexes], index=data_capex.index)

        op_scc = pd.Series([0] * len(indexes))
        constr_scc = pd.Series([0] * len(indexes))
        ...
        combined_constr = capex + constr_scc
        combined_op = opex + op_scc
        ...
        df_Economics = df_Economics.xs('costs', level='Perf_type')
        indexes, data_constr, data_op = prepare_dfs(df_impact, indexed_on, neg=True,
                                                    additional_costs=additional_costs,
                                                    scaling_factor=scaling_factor)
        indexes, data_capex, data_opex = prepare_dfs(df_Economics, indexed_on, neg=True,
                                                     additional_costs=additional_costs,
                                                     scaling_factor=scaling_factor)
        data_opex = data_opex.drop("avoided", level='type')
        data_op = data_op.drop("avoided", level='type')
        data_op[indexes] = data_op[indexes] * scc
        data_constr[indexes] = data_constr[indexes] * scc

        opex = data_opex[indexes].sum(axis=0).astype(int).reset_index(drop=True)
        capex = data_capex[indexes].sum(axis=0).astype(int).reset_index(drop=True)
        constr_scc = data_constr[indexes].sum(axis=0).astype(int).reset_index(drop=True)
        op_scc = data_op[indexes].sum(axis=0).astype(int).reset_index(drop=True)
        combined_op = opex + op_scc
        combined_constr = capex + constr_scc
    ...
    text_constr = ["<b>" + change_data.loc['x_axis_1', lang] + "</b><br>" + str(cp)
                   for cp in combined_constr]
    text_op = ["<b>" + change_data.loc['x_axis_2', lang] + "</b><br>" + str(op)
               for op in combined_op]
    pos_opex = data_opex[indexes][data_opex[indexes] > 0].sum(axis=0).astype(int).reset_index(drop=True) + \
               data_op[indexes][data_op[indexes] > 0].sum(axis=0).astype(int).reset_index(drop=True)

    fig = go.Figure()
    neg_opex = combined_op - pos_opex
    text_placeholder = 0.04 * max(max(combined_constr - neg_opex + combined_op),
                                  max(combined_constr + combined_op + neg_opex),
                                  max(combined_op))
    ...
    if add_annotation:
        for i in range(len(indexes)):
            ...
            fig.add_annotation(x=xtick[i], y=max(combined_constr[i], pos_opex[i],
                                                 combined_constr[i] + combined_op[i]) + text_placeholder,
                               text="<b>Total</b><br>" + str(
                                   combined_constr[i] + combined_op[i]) +
        ...
        if tech.loc[indexes].sum() > 0:
            fig.add_trace(
                go.Bar(name=tech[label], x=x1,
                       ...
                       showlegend=True))
        if data_constr.loc[line, indexes].sum() > 0:
            fig.add_trace(
                go.Bar(name='Impact - ' + data_constr.loc[line, label], x=x1,
                       y=data_constr.loc[line, indexes], width=1 / 3,
                       marker_color=tech["ColorPastel"],
                       hovertemplate='<b>' + data_constr.loc[line, label] + '</b>' +
                                     '<br>' + change_data.loc['keyword', lang] + ': %{y:.0f}' +
                                     change_data.loc['unites', lang],
    ...
    for line, layer in data_opex.iterrows():
        ...
        if abs(data_op.loc[line, indexes].sum()) > 0:
            fig.add_trace(
                go.Bar(name='Impact - ' + data_op.loc[line, label], x=x2,
                       y=data_op.loc[line, indexes], width=1 / 3,
                       marker_color=layer["ColorPastel"],
                       hovertemplate='<b>' + data_op.loc[line, label] + '</b>' +
                                     '<br>' + change_data.loc['keyword', lang] + ': %{y:.0f}' +
                                     change_data.loc['unites', lang],
                       marker_pattern_shape="x",
                       showlegend=False)
    ...
    fig.add_trace(
        go.Bar(
            ...
            width=1 / 6, showlegend=showlegend,
            legendgroup='group3',
...
def plot_actors(results, plot='costs', indexed_on='Scn_ID', label='EN_long', premium_version=None, per_m2=False,
                additional_costs={},
    ...
    indexes, data_capex, data_opex = prepare_dfs(df_Economics, indexed_on, neg=False, premium_version=premium_version,
                                                 additional_costs=additional_costs,
                                                 scaling_factor=scaling_factor)
