...
plotting.plot_performance(pareto, plot='costs', indexed_on='Pareto_ID', label='FR_long').show()
plotting.plot_performance(pareto, plot='gwp', indexed_on='Pareto_ID', label='FR_long').show()

# Pareto curve
plotting.plot_pareto(pareto, label='FR_long', color='ColorPastel').show()
...
sankey.plot_sankey(source, target, value, label_, color_).show()

# Hourly profiles (matplotlib ou plotly)
units_to_plot = ['ElectricalHeater', 'HeatPump', 'PV', 'Battery', 'EV_district']
plotting.plot_profiles(pareto['pareto'][4], units_to_plot, style='plotly', label='EN_long', color='ColorPastel', resolution='weekly').show()