...
param EV_pluged_out{p in Period, t in Time[p]} default 0.15;	# -
param EV_pluging_in{p in Period, t in Time[p]} default 0.15;	# -
param normalization_factor :=  max{p in PeriodStandard} ( sum{t in Time[p]}EV_pluging_in[p,t]);
...
param EV_displacement{u in UnitsOfType['EV'],p in Period, t in Time[p]} := EV_displacement_init[p] * EV_pluging_in[p,t] * n_vehicles;	#kW
...
EV_E_stored_plug_out[u,p,t] = EV_efficiency * EV_E_stored[u,p,prev(t,Time[p])] * EV_pluged_out[p,t];

subject to EV_EB_c2{u in UnitsOfType['EV'],p in Period,t in Time[p] diff {first(Time[p])}}:
EV_E_stored_plug_in[u,p,t] = EV_efficiency * EV_E_stored[u,p,prev(t,Time[p])] * (1-EV_pluged_out[p,t]) - 
...
EV_E_stored_plug_in[u,p,t] <= EV_limit_ch*EV_capacity*n_vehicles*Units_Use[u]* (1-EV_pluged_out[p,t]);											#kWh

subject to EV_c6b{u in UnitsOfType['EV'],p in Period,t in Time[p]}:
EV_E_stored_plug_in[u,p,t] >= EV_limit_di*EV_capacity*n_vehicles*Units_Use[u]* (1-EV_pluged_out[p,t]);											#kWh
...
EV_E_stored_plug_out[u,p,t] = EV_efficiency*EV_E_stored[u,p,last(Time[p])] * EV_pluged_out[p,t];


subject to EV_EB_cyclic2{u in UnitsOfType['EV'],p in Period,t in Time[p]:t=first(Time[p])}:
EV_E_stored_plug_in[u,p,t] = EV_efficiency*EV_E_stored[u,p,last(Time[p])] * (1-EV_pluged_out[p,t]) - EV_displacement[u,p,t]*Units_Use[u]*dt[p] +
											(EV_eff_ch*Units_demand['Electricity',u,p,t]  - (1/EV_eff_di)*Units_supply['Electricity',u,p,t])*dt[p];
