...
param CH4_stor_limit_max{u in UnitsOfType['CH4storage']} default 1;			#-	[2]
...
var CH4_stor_volume{u in UnitsOfType['CH4storage']} >= 0;
...
param compr_isentropic_eff_CH4 									:= 0.72;  		# Isentropic efficiency (-)
param R_const_CH4												:= 8.314;      	# Universal gas constant (J/(mol*K))
param T_compr_in_CH4 											:= 293.15;     	# Inlet temperature (K)
param M_CH4 													:= 16*1e-3;	   	# Molar mass of Ch4 (kg/mol)
param k_CH4 									  				:= 1.31;        # Heat capacity ratio for CH4
param LHV_CH4 									  				:= 49700;       # LHV of CH4 (kJ/kg)

# Compressibility factor (linear regression) valid for pressure [1 to 100 bars]
param Z_intercept_CH4 											:= 0.9984;
param Z_slope_CH4 	  											:= -0.0016;

param pressure_in_CH4 											default 15;	 # pressure at outlet of methanator
...
param Z_CH4_max 												:= Z_intercept_CH4 + Z_slope_CH4*CH4_stor_pressure;
...
subject to elec_CH4_calc{u in UnitsOfType['CH4storage'], p in Period,t in Time[p]}:
	Units_demand['Electricity',u,p,t] = Units_demand['Biomethane',u,p,t] / (LHV_CH4) * (R_const_CH4/M_CH4) * Z_CH4 * k_CH4 * T_compr_in_CH4
										/ (compr_isentropic_eff_CH4*(k_CH4-1)) * ((CH4_stor_pressure/pressure_in_CH4)^((k_CH4-1)/k_CH4) - 1)/1e3;
...
subject to CH4_stor_c1{u in UnitsOfType['CH4storage'], hy in Year}:
	Units_Mult[u] * CH4_stor_limit_max[u] >= CH4_stor_stored[u,hy];

subject to CH4_stor_c2{u in UnitsOfType['CH4storage'], hy in Year}:
	Units_Mult[u] * CH4_stor_limit_min[u] <= CH4_stor_stored[u,hy];

#--Storage volumes
subject to CH4_stor_c3{u in UnitsOfType['CH4storage'], hy in Year}:
	CH4_stor_volume[u] >= Units_Mult[u] * (3.6 * Z_CH4_max * R_const_CH4 * T_compr_in_CH4)/((LHV_CH4/1000) * M_CH4 * CH4_stor_pressure * 10^5);

subject to CH4_stor_c4{u in UnitsOfType['CH4storage'], hy in Year}:
	CH4_stor_volume[u] <= Units_Mult[u] * (3.6 * Z_CH4_max * R_const_CH4 * T_compr_in_CH4)/((LHV_CH4/1000) * M_CH4 * CH4_stor_pressure * 10^5);
