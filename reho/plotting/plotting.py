...
from reho.paths import path_to_clustering, path_to_plotting
...
def plot_expenses(results, plot='costs', indexed_on='Scn_ID', label='EN_long', add_annotation=True, per_m2=False, include_avoided=False, additional_costs=None,
                  additional_gwp=None,
                  scc=0.177,
    ...
    decimal = 0
    lang = re.split('_', label)[0]
    additional_costs = {} if additional_costs is None else additional_costs
    additional_gwp = {} if additional_gwp is None else additional_gwp
...
def plot_hourly_profiles(df_Results, units_to_plot, style='plotly', label='EN_long', color='ColorPastel', resolution='weekly', plot_curtailment=False,
    ...
    color: str
        Indicate the color set to use for the plot. 'ColorPastel' is default.
    resolution: str
        Controls how the profile is displayed. For line plots with a rolling average, choose among
        'hours' (no smoothing), 'daily', 'weekly' (default), or 'monthly'. For bar plots that sum
        energy by period, choose 'day', 'week', or 'month'. In all cases the x-axis shows month labels.
    ...
    pd.DataFrame
        (Optional) A dataframe for further post-processing or reporting purposes.
    """
    _lang = 'FR' if label.startswith('FR') else 'EN'
    _t = _TRANSLATIONS[_lang]
    _MONTH_NAMES = _t['months']
    ...
    if items_average > 1:
        import_profile_raw = {layer: import_profile[layer].copy() for layer in layers}
        export_profile_raw = {layer: export_profile[layer].copy() for layer in layers}
        demands_raw = {unit: demands[unit].copy() for unit in units_demand}
        supplies_raw = {unit: supplies[unit].copy() for unit in units_supply}
        if plot_curtailment and 'PV' in curtailments:
            curtailments_pv_raw = curtailments['PV'].copy()
    ...
    n = len(import_profile["Electricity"])
    x_data = list(range(1, n + 1))
    x_raw = list(range(1, 8761))
    _hover_dates = []
    for _h in range(8760):
        _d = datetime.date(2001, 1, 1) + datetime.timedelta(days=_h // 24)
        _hover_dates.append(f"{_d.day} {_MONTH_NAMES[_d.month - 1]}, {(_h % 24):02d}:00")
    _hover_dates_data = _hover_dates[:n]
    month_tick_vals, month_start_vals, pos = [], [], 1
    for h in _MONTH_HOURS:
        month_start_vals.append(pos)
        month_tick_vals.append(pos + h // 2)
        pos += h
    ...
    if style == 'matplotlib':
        fig, ax = plt.subplots()
        if items_average > 1:
            ax.plot(x_raw, import_profile_raw["Electricity"], color=layout.loc['Electricity_import', color], alpha=0.3)
            ax.plot(x_raw, -export_profile_raw["Electricity"], color=layout.loc['Electricity_export', color], alpha=0.3)
            for layer in list(layers)[1:]:
                ax.plot(x_raw, import_profile_raw[layer], color=layout.loc[layer, color], alpha=0.3)
            for unit in units_demand:
                ax.plot(x_raw, demands_raw[unit], color=layout.loc[unit, color], alpha=0.3)
            for unit in units_supply:
                ax.plot(x_raw, -supplies_raw[unit], color=layout.loc[unit, color], alpha=0.3)
            if plot_curtailment:
                ax.plot(x_raw, -curtailments_pv_raw, color=layout.loc['Curtailment', color], alpha=0.3)
        ax.plot(x_data, import_profile["Electricity"], color=layout.loc['Electricity_import', color],
                label=layout.loc['Electricity_import', label])
        ax.plot(x_data, -export_profile["Electricity"], color=layout.loc['Electricity_export', color],
                label=layout.loc['Electricity_export', label])
        for layer in list(layers)[1:]:
            ax.plot(x_data, import_profile[layer], color=layout.loc[layer, color],
                    label=layout.loc[layer, label], alpha=0.5)
        for unit in units_demand:
            ax.plot(x_data, demands[unit], linestyle='--', label=layout.loc[unit, label],
                    color=layout.loc[unit, color])
        for unit in units_supply:
            ax.plot(x_data, -supplies[unit], label=layout.loc[unit, label],
                    color=layout.loc[unit, color])
        if plot_curtailment:
            ax.plot(x_data, -curtailments['PV'], linestyle='.', label=layout.loc['Curtailment', label],
                    color=layout.loc['Curtailment', color])
        for x_start in month_start_vals[1:]:
            ax.axvline(x=x_start, color='lightgrey', linewidth=0.8, linestyle='--')
        ax.set_xticks(month_tick_vals)
        ...
        _ht = '<b>%{fullData.name}</b><br>%{customdata}<br>%{y:.1f} kWh<extra></extra>'

        if items_average > 1:
            fig.add_trace(go.Scatter(
                x=x_raw, y=import_profile_raw["Electricity"], mode="lines",
                showlegend=False, opacity=0.3, hoverinfo='skip',
                line=dict(color=layout.loc['Electricity_import', color])
            ))
            if export_profile_raw["Electricity"].any() > 0:
                fig.add_trace(go.Scatter(
                    x=x_raw, y=-export_profile_raw["Electricity"], mode="lines",
                    showlegend=False, opacity=0.3, hoverinfo='skip',
                    line=dict(color=layout.loc['Electricity_export', color])
                ))
            for layer in list(layers)[1:]:
                if import_profile_raw[layer].any() > 0:
                    fig.add_trace(go.Scatter(
                        x=x_raw, y=import_profile_raw[layer], mode="lines",
                        showlegend=False, opacity=0.3, hoverinfo='skip',
                        line=dict(color=layout.loc[layer + '_import', color])
                    ))
            for unit in units_demand:
                if demands_raw[unit].any() > 0:
                    fig.add_trace(go.Scatter(
                        x=x_raw, y=demands_raw[unit], mode="lines",
                        showlegend=False, opacity=0.3, hoverinfo='skip',
                        line=dict(color=layout.loc[unit, color])
                    ))
            for unit in units_supply:
                if supplies_raw[unit].any() > 0:
                    fig.add_trace(go.Scatter(
                        x=x_raw, y=-supplies_raw[unit], mode="lines",
                        showlegend=False, opacity=0.3, hoverinfo='skip',
                        line=dict(color=layout.loc[unit, color])
                    ))
            if plot_curtailment:
                fig.add_trace(go.Scatter(
                    x=x_raw, y=-curtailments_pv_raw, mode="lines",
                    showlegend=False, opacity=0.3, hoverinfo='skip',
                    line=dict(color=layout.loc['Curtailment', color])
                ))

        fig.add_trace(go.Scatter(
            x=x_data, y=import_profile["Electricity"], mode="lines",
            name=layout.loc['Electricity_import', label],
            customdata=_hover_dates_data, hovertemplate=_ht,
            line=dict(color=layout.loc['Electricity_import', color], width=2)
        ))
        if export_profile["Electricity"].any() > 0:
            fig.add_trace(go.Scatter(
                x=x_data, y=-export_profile["Electricity"], mode="lines",
                name=layout.loc['Electricity_export', label],
                customdata=_hover_dates_data, hovertemplate=_ht,
                line=dict(color=layout.loc['Electricity_export', color], dash='dash', width=2)
            ))
        for layer in list(layers)[1:]:
            if import_profile[layer].any() > 0:
                fig.add_trace(go.Scatter(
                    x=x_data, y=import_profile[layer], mode="lines",
                    name=layout.loc[layer + '_import', label],
                    customdata=_hover_dates_data, hovertemplate=_ht,
                    line=dict(color=layout.loc[layer + '_import', color], width=2)
                ))
        for unit in units_demand:
            if demands[unit].any() > 0:
                fig.add_trace(go.Scatter(
                    x=x_data, y=demands[unit], mode="lines",
        ...
        for unit in units_supply:
            if supplies[unit].any() > 0:
                fig.add_trace(go.Scatter(
                    x=x_data, y=-supplies[unit], mode="lines",
                    name=layout.loc[unit, label],
                    customdata=_hover_dates_data, hovertemplate=_ht,
                    line=dict(color=layout.loc[unit, color], dash='dash', width=2)
                ))
        if plot_curtailment:
            fig.add_trace(go.Scatter(
                x=x_data, y=-curtailments['PV'], mode="lines",
                name=layout.loc['Curtailment', label],
                customdata=_hover_dates_data, hovertemplate=_ht,
                line=dict(color=layout.loc['Curtailment', color], dash='dot', width=2)
            ))
        for x_start in month_start_vals[1:]:
            fig.add_vline(x=x_start, line=dict(color='lightgrey', width=1, dash='dot'))
        fig.update_layout(
            template='plotly_white',
            xaxis=dict(tickvals=month_tick_vals, ticktext=_MONTH_NAMES, showgrid=False),
            xaxis_title=obj_x,
            yaxis_title=obj_y
        )
...
def plot_daily_profiles(df_Results, units_to_plot, period, season_name=None, style='plotly', label='EN_long', color='ColorPastel',
                        plot_curtailment=False, title=None, filename=None, export_format='html', return_df=False):
    """
    Plots a 24-hour profile for a single typical period.
    ...
    units_to_plot: list
        Units to be plotted.
    period:
        Typical period identifier to plot (as found in df_Results['df_Index'].PeriodOfYear).
    style: str
        Choose between 'plotly' or 'matplotlib'.
    label: str
        Indicate the language to use for the plot. Choose among 'FR_long', 'FR_short', 'EN_long', 'EN_short'.
    ...
    Returns
    ----------
    plotly.graph_objs.Figure or matplotlib.pyplot
    """
    units_demand = []
    units_supply = []
    for unit in units_to_plot:
    ...
    export_day = {}
    for layer in layers:
        grid = df_Results['df_Grid_t'].xs((layer, 'Network'), level=('Layer', 'Hub'))
        import_day[layer] = grid.Grid_supply[:-2].xs(period, level='Period').values
        export_day[layer] = grid.Grid_demand[:-2].xs(period, level='Period').values

    demands_day = {}
    supplies_day = {}
    curtailments_day = {}
    for unit in units_to_plot:
        df_agg = df_Results['df_Unit_t'][df_Results['df_Unit_t'].index.get_level_values('Unit').str.contains(unit)]
        if unit in units_demand:
            demands_day[unit] = df_agg.droplevel('Layer').Units_demand[:-2].groupby(['Period', 'Time']).sum().xs(period, level='Period').values
        if unit in units_supply:
            supplies_day[unit] = df_agg.droplevel('Layer').Units_supply[:-2].groupby(['Period', 'Time']).sum().xs(period, level='Period').values
        if unit == 'PV' and plot_curtailment:
            curtailments_day['PV'] = df_agg.droplevel('Layer').Units_curtailment[:-2].groupby(['Period', 'Time']).sum().xs(period, level='Period').values

    _lang = 'FR' if label.startswith('FR') else 'EN'
    _t = _TRANSLATIONS[_lang]
    n_hours = len(import_day["Electricity"])
    x_day = list(range(1, n_hours + 1))
    obj_x = _t['time_h']
    obj_y = _t['kwh']
    _prefix = f"{season_name}, " if season_name else ""
    _hover_times = [f"{_prefix}{h:02d}:00" for h in range(n_hours)]
    _ht_daily = '<b>%{fullData.name}</b><br>%{customdata}<br>%{y:.1f} kWh<extra></extra>'

    if style == 'matplotlib':
        fig, ax = plt.subplots()
        ax.plot(x_day, import_day["Electricity"], color=layout.loc['Electricity_import', color],
                label=layout.loc['Electricity_import', label])
        ax.plot(x_day, -export_day["Electricity"], color=layout.loc['Electricity_export', color],
                label=layout.loc['Electricity_export', label])
        for layer in list(layers)[1:]:
            ax.plot(x_day, import_day[layer], color=layout.loc[layer, color],
                    label=layout.loc[layer, label], alpha=0.5)
        for unit in units_demand:
            ax.plot(x_day, demands_day[unit], linestyle='--', label=layout.loc[unit, label],
                    color=layout.loc[unit, color])
        for unit in units_supply:
            ax.plot(x_day, -supplies_day[unit], label=layout.loc[unit, label],
                    color=layout.loc[unit, color])
        if plot_curtailment:
            ax.plot(x_day, -curtailments_day['PV'], linestyle='.', label=layout.loc['Curtailment', label],
                    color=layout.loc['Curtailment', color])
        ax.set_title(title)
        ax.set_xlabel(obj_x)
        ax.set_ylabel(obj_y)
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=x_day, y=import_day["Electricity"], mode="lines",
            name=layout.loc['Electricity_import', label],
            customdata=_hover_times, hovertemplate=_ht_daily,
            line=dict(color=layout.loc['Electricity_import', color])
        ))
        if export_day["Electricity"].any() > 0:
            fig.add_trace(go.Scatter(
                x=x_day, y=-export_day["Electricity"], mode="lines",
                name=layout.loc['Electricity_export', label],
                customdata=_hover_times, hovertemplate=_ht_daily,
                line=dict(color=layout.loc['Electricity_export', color], dash='dash')
            ))
        for layer in list(layers)[1:]:
            if import_day[layer].any() > 0:
                fig.add_trace(go.Scatter(
                    x=x_day, y=import_day[layer], mode="lines",
                    name=layout.loc[layer + '_import', label],
                    customdata=_hover_times, hovertemplate=_ht_daily,
                    line=dict(color=layout.loc[layer + '_import', color])
                ))
        for unit in units_demand:
            if demands_day[unit].any() > 0:
                fig.add_trace(go.Scatter(
                    x=x_day, y=demands_day[unit], mode="lines",
                    name=layout.loc[unit, label],
                    customdata=_hover_times, hovertemplate=_ht_daily,
                    line=dict(color=layout.loc[unit, color])
                ))
        for unit in units_supply:
            if supplies_day[unit].any() > 0:
                fig.add_trace(go.Scatter(
                    x=x_day, y=-supplies_day[unit], mode="lines",
                    name=layout.loc[unit, label],
                    customdata=_hover_times, hovertemplate=_ht_daily,
        ...
        if plot_curtailment:
            fig.add_trace(go.Scatter(
                x=x_day, y=-curtailments_day['PV'], mode="lines",
                name=layout.loc['Curtailment', label],
                customdata=_hover_times, hovertemplate=_ht_daily,
...
def plot_combined_profiles(df_Results, units_to_plot, label='EN_long', color='ColorPastel', resolution='weekly',
                           plot_curtailment=False, title=None, filename=None, export_format='html'):
    """
    Combines an annual hourly profile with four seasonal typical-day profiles in a single figure.

    The left panel shows the full-year profile (see plot_hourly_profiles). The right panel is a
    2x2 grid of typical days: Winter (Jan 15), Spring (Apr 15), Summer (Jul 15), Autumn (Oct 15).
    ...
    color: str
        Indicate the color set to use for the plot. 'ColorPastel' is default.
    resolution: str
        Rolling average for the annual profile. See plot_hourly_profiles.
    plot_curtailment: bool
        PV curtailment can optionally be plotted.
    title: str
    ...
    season_days = {'Winter': 15, 'Spring': 105, 'Summer': 196, 'Autumn': 288}
    season_periods = {
        season: df_Results['df_Index'].PeriodOfYear[day * 24]
        for season, day in season_days.items()
    }

    hourly_fig = plot_hourly_profiles(df_Results, units_to_plot, style='plotly', label=label, color=color,
                                      resolution=resolution, plot_curtailment=plot_curtailment)
    daily_figs = {
        season: plot_daily_profiles(df_Results, units_to_plot, period=period,
                                    season_name=_t['seasons'][season], style='plotly',
                                    label=label, color=color, plot_curtailment=plot_curtailment)
        for season, period in season_periods.items()
    }

    season_labels = [_t['seasons'][s] for s in season_days]
    combined = make_subplots(
        rows=2, cols=3,
        specs=[[{"rowspan": 2}, {}, {}], [None, {}, {}]],
        column_widths=[0.5, 0.25, 0.25],
        subplot_titles=[_t['annual']] + season_labels,
    )

    for trace in hourly_fig.data:
        combined.add_trace(copy.deepcopy(trace), row=1, col=1)

    season_positions = [('Winter', 1, 2), ('Spring', 1, 3), ('Summer', 2, 2), ('Autumn', 2, 3)]
    for season, row, col in season_positions:
        for trace in daily_figs[season].data:
            t = copy.deepcopy(trace)
            t.showlegend = False
            combined.add_trace(t, row=row, col=col)

    for shape in hourly_fig.layout.shapes:
        combined.add_shape(copy.deepcopy(shape), row=1, col=1)

    hourly_xaxis = hourly_fig.layout.xaxis
    combined.update_layout(
        template='plotly_white',
        title=title,
        xaxis=dict(tickvals=hourly_xaxis.tickvals, ticktext=hourly_xaxis.ticktext, showgrid=False,
                   title=_t['month']),
        yaxis=dict(title=_t['kwh']),
    )
    combined.update_xaxes(title_text=_t['time_h'], row=1, col=2)
    combined.update_xaxes(title_text=_t['time_h'], row=1, col=3)
    combined.update_xaxes(title_text=_t['time_h'], row=2, col=2)
    combined.update_xaxes(title_text=_t['time_h'], row=2, col=3)

    if filename is not None:
        if not os.path.isdir(os.path.dirname(filename)):
            os.makedirs(os.path.dirname(filename))
        if export_format == 'html':
            combined.write_html(filename + '.' + export_format)
        if export_format in ('png', 'pdf'):
            combined.write_image(filename + '.' + export_format)
...
def plot_eud(results, label='EN_long', title=None, filename=None, export_format='html', scaling_factor=1, return_df=False):
    ...
    if return_df:
        logger.error('Sunburst parents (%s) and children (%s) have different lengths: %s vs %s.',
                     parents_name, child_name, len(parents_name), len(child_name))
        df = pd.DataFrame()
...
def plot_pareto_by_objectives(results, objectives=None, style='plotly', annotation="TOTEX", title=None, filename=None, export_format='png'):
    """
    ...
    plotly.graph_objs.Figure
        The generated plotly figure.
    """
    objectives = ["CAPEX", "OPEX"] if objectives is None else objectives
...
def plot_composite_curve(df_Results, cluster, periods=None, filename=None, export_format='png', return_df=False):
    """
    ...
    pd.DataFrame
        (Optional) A dataframe for further post-processing or reporting purposes.
    """
    periods = ["Yearly"] if periods is None else periods
...
def plot_electricity_flows(df_Results, color='ColorPastel', day_of_the_year=1, time_range='week', label='EN_long'):
    ...
    try:
        ...
        # No inter-period storage in these results: nothing to overlay on the plot.
        IP_storage = None
