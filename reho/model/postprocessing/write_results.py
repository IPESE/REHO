...
def get_df_Results_from_SP(ampl, scenario, method, buildings_data, filter=True):
    ...
    def set_df_performance(ampl, scenario):
        df1 = get_parameter_in_pandas(ampl, 'Costs_House_op')  # without the comfort penalty costs
        df1 = df1.rename(columns={'Costs_House_op': 'Costs_op'})

        df2 = get_parameter_in_pandas(ampl, 'Costs_House_inv')
        ...
        df2['Costs_grid_connection'] = get_parameter_in_pandas(ampl, 'Costs_grid_connection_House', multi_index=True).groupby(level=1).sum()  # yearly cost for grid connection

        df3 = get_parameter_in_pandas(ampl, 'Costs_House_rep')
        df3['Costs_House_rep'] = df3['Costs_House_rep'] * tau[0]
        df3 = df3.rename(columns={'Costs_House_rep': 'Costs_rep'})

        df4 = get_parameter_in_pandas(ampl, 'Costs_House_cft')
        df4 = df4.rename(columns={'Costs_House_cft': 'Costs_ft'})

        df5 = get_parameter_in_pandas(ampl, 'GWP_house_op')
        df5 = df5.rename(columns={'GWP_house_op': 'GWP_op'})

        df6 = get_parameter_in_pandas(ampl, 'GWP_house_constr')
        df6 = df6.rename(columns={'GWP_house_constr': 'GWP_constr'})

        df71 = get_parameter_in_pandas(ampl, 'EMOO_CAPEX')
        df72 = get_parameter_in_pandas(ampl, 'EMOO_OPEX')
        df73 = get_parameter_in_pandas(ampl, 'EMOO_TOTEX')
        df75 = get_parameter_in_pandas(ampl, 'EMOO_GWP')
        df76 = get_parameter_in_pandas(ampl, 'EMOO_grid')

        df_N1 = get_parameter_in_pandas(ampl, 'Costs_op')  # without the comfort penalty costs
        df_N2 = get_parameter_in_pandas(ampl, 'Costs_inv')
        df_N2['Costs_inv'] = df_N2['Costs_inv'] * tau[0]
        df_N2['ANN_factor'] = tau[0]
        df_N2['Costs_grid_connection'] = get_parameter_in_pandas(ampl, 'Costs_grid_connection').sum().values/2  # TODO enhance
        df_N3 = get_parameter_in_pandas(ampl, 'Costs_rep')
        df_N3['Costs_rep'] = df_N3['Costs_rep'] * tau[0]
        df_N4 = pd.DataFrame({'Costs_ft': [df4.sum()['Costs_ft']]})
        df_N5 = get_parameter_in_pandas(ampl, 'GWP_op')
        df_N6 = get_parameter_in_pandas(ampl, 'GWP_constr')
        ...
        df_Epsilon['Objective'] = get_parameter_in_pandas(ampl, scenario["Objective"]).values[0][0]- df_N4.values[0][0]
    ...
    def set_df_annuals(ampl):
        # Annuals
        df1 = get_parameter_in_pandas(ampl, 'AnnualNetwork_demand')
        df1.columns = ['Demand_MWh']
        df1 = pd.concat([df1], keys=['Network'])
        df1.index = df1.index.reorder_levels([1, 0])
        df2 = get_parameter_in_pandas(ampl, 'AnnualNetwork_supply')
        ...
        df3 = get_parameter_in_pandas(ampl, 'AnnualDomestic_electricity')
        df3 = df3.set_index([pd.Index(["Electricity"]*df3.index.size), df3.index])
        df3.columns = ['Demand_MWh']
        df4 = get_parameter_in_pandas(ampl, 'AnnualHouse_Q')
        df4.columns = ['Demand_MWh']
        df5 = get_parameter_in_pandas(ampl, 'AnnualHeatGainHouse')
        df5.columns = ['Supply_MWh']
        df5 = pd.concat([df5], keys=['HeatGains'])
        df6 = get_parameter_in_pandas(ampl, 'AnnualSolarGainHouse')
        df6.columns = ['Supply_MWh']
        df6 = pd.concat([df6], keys=['SolarGains'])
        df3456 = pd.concat([df3, df4, df5, df6], sort=False)

        df7 = get_parameter_in_pandas(ampl, 'AnnualUnit_in')
        df7.columns = ['Demand_MWh']
        df8 = get_parameter_in_pandas(ampl, 'AnnualUnit_out')
        df8.columns = ['Supply_MWh']
        df78 = pd.concat([df7, df8], axis=1, sort=False)
        df9 = get_parameter_in_pandas(ampl, 'AnnualUnit_Q')
    ...
    def set_df_unit(ampl):
        # Unit
        tau = ampl.getParameter('tau').getValues().toList()
        df1 = get_parameter_in_pandas(ampl, 'Units_Use')
        df2 = get_parameter_in_pandas(ampl, 'Units_Mult')

        df3 = tau[0] * get_parameter_in_pandas(ampl, 'Costs_Unit_inv')
        df4 = get_parameter_in_pandas(ampl, 'GWP_Unit_constr') # per year! For total- multiply with lifetime
        df5 = get_parameter_in_pandas(ampl, 'lifetime')
        df_Unit = pd.concat([df1, df2, df3, df4, df5], axis=1)
        df_Unit.index.names = ['Unit']
        df_Unit = df_Unit.sort_index()

        # Unit_t
        df1 = get_parameter_in_pandas(ampl, 'Units_demand', multi_index=True)
        df2 = get_parameter_in_pandas(ampl, 'Units_supply', multi_index=True)
        df3 = get_parameter_in_pandas(ampl, 'Units_curtailment', multi_index=True)
        df4 = get_parameter_in_pandas(ampl, 'BAT_E_stored', multi_index=True)
        ...
        if "EV_district" in [unit for unit, value in ampl.getVariable('Units_Use').instances()]:
            df5 = get_parameter_in_pandas(ampl, 'EV_E_stored', multi_index=True)
    ...
    def set_df_grid(ampl, method):
        # Grid_t
        df1 = get_parameter_in_pandas(ampl, 'Grid_demand', multi_index=True)
        df2 = get_parameter_in_pandas(ampl, 'Grid_supply', multi_index=True)
        ...
        if not method["district-scale"] or not method["district-scale"]:
            df3 = get_parameter_in_pandas(ampl, 'Network_demand', multi_index=True)
            df3.columns = ['Grid_demand']
            df4 = get_parameter_in_pandas(ampl, 'Network_supply', multi_index=True)
            df4.columns = ['Grid_supply']
            df_cs_net = get_parameter_in_pandas(ampl, 'Cost_supply_network', multi_index=True)
            df_cs_net.columns = ['Cost_supply']
            df_cd_net = get_parameter_in_pandas(ampl, 'Cost_demand_network', multi_index=True)
    ...
    def set_df_buildings_t(ampl):
        ...
        delta_T = get_parameter_in_pandas(ampl, 'DHW_dT').iloc[0,0]
        ...
        df32 = get_parameter_in_pandas(ampl, 'House_Q_heating')
        df33 = get_parameter_in_pandas(ampl, 'House_Q_cooling')
    ...
    def set_df_stream_t(ampl):
        ...
        df_Q = get_parameter_in_pandas(ampl, 'Streams_Q', multi_index=True)
        df_Q.index.set_names(['Service', 'Stream', 'Period', 'Time'], inplace=True)

        df1 = get_parameter_in_pandas(ampl, 'HC_Streams_Mult', multi_index=True)
        ...
        df_Tmin = get_parameter_in_pandas(ampl, 'dTmin')
    ...
    def set_dfs_lca(ampl):
        ...
        LCA_units = get_parameter_in_pandas(ampl, 'lca_units')
        LCA_units = LCA_units.stack().unstack(level=0).droplevel(level=1)

        LCA_tot = get_parameter_in_pandas(ampl, 'lca_tot')
        LCA_tot = LCA_tot.stack().unstack(level=0)
        LCA_tot.index = ["Network"]
        LCA_tot_house = get_parameter_in_pandas(ampl, 'lca_tot_house')
        ...
        LCA_op = get_parameter_in_pandas(ampl, 'lca_op')
    ...
    def set_dfs_pv(ampl):
        ...
        # ["df_PV_orientation"]
        df_PVA_module_nbr = get_parameter_in_pandas(ampl, 'PVA_module_nbr', multi_index=True)
        ...
        # df_PV_Surface_profiles = get_parameter_in_pandas(ampl, "PV_electricity")
        # df_PV_Surface_loss = get_parameter_in_pandas(ampl, "PV_electricity_without_loss")
    ...
    def set_dfs_other(ampl):
        # Time
        df1 = get_parameter_in_pandas(ampl, 'dp')
        df2 = get_parameter_in_pandas(ampl, 'TimeEnd')
        df3 = get_parameter_in_pandas(ampl, 'dt')
        ...
        # Index
        df_Index = get_parameter_in_pandas(ampl, 'PeriodOfYear')
...
def get_df_Results_from_MP(ampl, binary=False, method=None, district=None, read_DHN=False, scenario={}):
    ...
    # District
    df1 = get_parameter_in_pandas(ampl, 'Costs_House_op')
    df2 = get_parameter_in_pandas(ampl, 'Costs_House_inv')
    df3 = get_parameter_in_pandas(ampl, 'Costs_House_cft')
    df4 = df1.values + df2
    ...
    if read_DHN:
        df7 = get_parameter_in_pandas(ampl, 'diameter_max')
        df8 = get_parameter_in_pandas(ampl, 'DHN_inv_house')
        df8.columns = ["DHN_inv"]
        df9 = get_parameter_in_pandas(ampl, 'flowrate_max')
        df_House = pd.concat([df_House, df7, df8, df9], axis=1)


    df1 = get_parameter_in_pandas(ampl, 'Costs_op')
    df2 = get_parameter_in_pandas(ampl, 'Costs_inv')  # with comfort costs
    df3 = get_parameter_in_pandas(ampl, 'Costs_cft')  # with comfort costs
    df4 = get_parameter_in_pandas(ampl, 'Costs_tot')  # with comfort costs
    df5 = get_parameter_in_pandas(ampl, 'GWP_op')
    df6 = get_parameter_in_pandas(ampl, 'GWP_constr')
    df_District = pd.concat([df1, df2, df3, df4, df5, df6], axis=1)
    if read_DHN:
        df7 = np.sqrt(np.sum(df_House[["diameter_max"]]**2)).to_frame().transpose()
        df8 = get_parameter_in_pandas(ampl, 'DHN_inv')
    ...
    df5 = get_parameter_in_pandas(ampl, 'Network_supply', multi_index=True)
    df6 = get_parameter_in_pandas(ampl, 'Network_demand', multi_index=True)
    ...
    df_Unit = pd.concat([df1, df2, df3, df4, df5], axis=1)
    if read_DHN:
        df_DHN = pd.DataFrame([[1, 1, get_parameter_in_pandas(ampl, 'DHN_inv')["DHN_inv"][0], 0, 0]], index=["DHN"], columns=df_Unit.columns)
    ...
    df1 = get_parameter_in_pandas(ampl, 'Units_demand', multi_index=True)
    df2 = get_parameter_in_pandas(ampl, 'Units_supply', multi_index=True)
    df3 = get_parameter_in_pandas(ampl, 'BAT_E_stored', multi_index=True)
    ...
    if len(district.UnitsOfDistrict) > 0:
        if "EV_district" in district.UnitsOfDistrict:
            df4 = get_parameter_in_pandas(ampl, 'EV_E_stored', multi_index=True)
            df4 = pd.concat([df4], keys=['Electricity'], names=['Layer'])
    ...
    # df_lca
    if method["save_lca"]:
        LCA_units = get_parameter_in_pandas(ampl, 'lca_units', multi_index=True)
        LCA_units = LCA_units.stack().unstack(level=0).droplevel(level=1)
        ...
        LCA_tot = LCA_tot.stack().unstack(level=0)
        LCA_tot.index = ["Network"]
        LCA_tot_house = get_parameter_in_pandas(ampl, 'lca_tot_house', multi_index=True)
        LCA_tot_house = LCA_tot_house.stack().unstack(level=0).droplevel(1)
        df_Results["df_lca_Performance"] = pd.concat([LCA_tot_house, LCA_tot], axis=0)
        df_Results["df_lca_Performance"].index.names = ['Hub']

        LCA_op = get_parameter_in_pandas(ampl, 'lca_op', multi_index=True)
        LCA_op = LCA_op.stack().unstack(level=0).droplevel(level=1)
        df_Results["df_lca_operation"] = LCA_op

    if method["actors_cost"]:
        df1 = get_parameter_in_pandas(ampl, 'Cost_demand_district', multi_index=True).groupby(level=(0,2)).sum()
        df2 = get_parameter_in_pandas(ampl, 'Cost_supply_district', multi_index=True).groupby(level=(0,2)).sum()
        df3 = get_parameter_in_pandas(ampl, 'Cost_self_consumption', multi_index=True).groupby(level=1).sum()
        df3 = pd.concat({'Electricity': df3})
        df_Results["df_Actors_tariff"] = pd.concat([df1, df2, df3], axis=1)

        df_Results["df_Actors"] = get_parameter_in_pandas(ampl, 'objective_functions')

        df1 = get_parameter_in_pandas(ampl, 'C_op_renters_to_utility')
        df2 = get_parameter_in_pandas(ampl, 'C_op_renters_to_owners')
        df3 = get_parameter_in_pandas(ampl, 'C_op_utility_to_owners')
        df4 = get_parameter_in_pandas(ampl, 'Costs_House_inv')
        df4.columns = ["owner_inv"]
        df5 = get_parameter_in_pandas(ampl, 'owner_portfolio')
...
def get_parameter_in_pandas(ampl, ampl_name, multi_index=False):
