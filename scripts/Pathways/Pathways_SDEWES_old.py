...
if __name__ == '__main__':
    ...
    EMOO_list_PV, y_span = reho_initial.get_logistic(E_start=PV_init_int, E_stop=PV_tot_int, y_start=y_start,y_stop=y_stop, k_factor=k, c_factor=c, n_steps=N_iter_pathway, final_value=False,starting_value=True)
    ...
    # Note: The possible PV capacities are given by the enforce_PV_max scenario above. However, the already installed capacities do not correspond necessary to these maximum values. It is therefore necessary to modify the list.
    df_PV_installable = reho_max_PV.loc[(reho_max_PV.index.str.contains('PV')) & ~(reho_max_PV.index.str.contains('district'))]  # What the max scenario gave; TO DO: make that the dataframe only guives the name of the unit and the Units_Mult
    ...
    pathway_parameters['y_span'] = y_span
    ...
    #pathway_data_2['EMOO']['PV']['Units_Mult'], temp, pathway_data_2['EMOO']['PV']['Units_Use'] = reho_initial.select_values_random(initial_selection=initial_selection_PV,steps=EMOO_list_PV,values=installable_PV_future)  # Random attribution
