...
param EV_charger_Power default 11;			#kW	 	[5] and [7]
param EV_capacity default 70;		#kWh 	[5] and [6]
param EV_plug_out{p in Period, t in Time[p]} default 0.15;	# -
param normalization_factor :=  max{p in PeriodStandard} ( sum{t in Time[p]}EV_plug_out[p,t]);
...
param EV_displacement{u in UnitsOfType['EV'],p in Period, t in Time[p]} := EV_displacement_init[p]*EV_plug_out[p,t];	#kW
...
EV_E_stored_plug_out[u,p,t] = EV_efficiency*EV_E_stored[u,p,prev(t,Time[p])] * EV_plug_out[p,t] - EV_displacement[u,p,t]*n_vehicles*Units_Use[u]*dt[p];

subject to EV_EB_c2{u in UnitsOfType['EV'],p in Period,t in Time[p] diff {first(Time[p])}}:
EV_E_stored_plug_in[u,p,t] = EV_efficiency*EV_E_stored[u,p,prev(t,Time[p])] * (1-EV_plug_out[p,t]) +
...
EV_E_stored_plug_in[u,p,t] <= EV_limit_ch*EV_capacity*n_vehicles*Units_Use[u]* (1-EV_plug_out[p,t]);											#kWh

subject to EV_c6b{u in UnitsOfType['EV'],p in Period,t in Time[p]}:
EV_E_stored_plug_in[u,p,t] >= EV_limit_di*EV_capacity*n_vehicles*Units_Use[u]* (1-EV_plug_out[p,t]);											#kWh
	
	
subject to EV_c5c{u in UnitsOfType['EV'],p in Period,t in Time[p]}:
EV_E_stored_plug_out[u,p,t] <= EV_limit_ch*EV_capacity*n_vehicles*Units_Use[u]* EV_plug_out[p,t];											#kWh

subject to EV_c6c{u in UnitsOfType['EV'],p in Period,t in Time[p]}:
EV_E_stored_plug_out[u,p,t] >= EV_limit_di*EV_capacity*n_vehicles*Units_Use[u]* EV_plug_out[p,t];											#kWh
																			#-
...
EV_E_stored_plug_out[u,p,t] = EV_efficiency*EV_E_stored[u,p,last(Time[p])] * EV_plug_out[p,t] - EV_displacement[u,p,t]*Units_Use[u]*n_vehicles*dt[p];


subject to EV_EB_cyclic2{u in UnitsOfType['EV'],p in Period,t in Time[p]:t=first(Time[p])}:
EV_E_stored_plug_in[u,p,t] = EV_efficiency*EV_E_stored[u,p,last(Time[p])] * (1-EV_plug_out[p,t]) +
											(EV_eff_ch*Units_demand['Electricity',u,p,t]  - (1/EV_eff_di)*Units_supply['Electricity',u,p,t])*dt[p];
