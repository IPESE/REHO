...
param max_distperday default 3; # pkm moyenne mobilité douce : 2.8km per day [1] T 3.3.1.1
param n_bikesperhab default 0.9;
param max_n_bikes := n_bikesperhab * Population;
param max_share_bikes default 0.1; # [1] G 3.3.1.6 : share of bikes and walking amounts to ~ 8%
...
subject to Bikes_profile{u in UnitsOfType['Bike'],p in Period, t in Time[p]}:
Units_supply['Mobility',u,p,t] = share_bike[u,p] * Daily_Profile[u,p,t];

subject to Bikes_maxshare{p in Period}:
sum {u in UnitsOfType['Bike']} (share_bike[u,p]) <= max_share_bikes;