...
subject to EV_energy_balance{u in UnitsOfType['EV'],p in Period,t in Time[p] diff {first(Time[p])}}:
...
Units_demand['Electricity',u,p,t] <= EV_charger_Power * n_vehicles * Units_Use[u];

subject to EV_c2{u in UnitsOfType['EV'],p in Period,t in Time[p]}:
Units_supply['Electricity',u,p,t] <= EV_charger_Power * n_vehicles * Units_Use[u];


subject to EV_c5a{u in UnitsOfType['EV'],p in Period,t in Time[p]}:
EV_E_stored[u,p,t] <= EV_limit_ch * EV_capacity * n_vehicles * Units_Use[u];

subject to EV_c6a{u in UnitsOfType['EV'],p in Period,t in Time[p]}:
EV_E_stored[u,p,t] >= EV_limit_di * EV_capacity * n_vehicles * Units_Use[u];


subject to EV_c5b{u in UnitsOfType['EV'],p in Period,t in Time[p]}:
EV_E_stored_plug_in[u,p,t] <= EV_limit_ch*EV_capacity*n_vehicles*Units_Use[u]* (1-EV_plugged_out[p,t]);

subject to EV_c6b{u in UnitsOfType['EV'],p in Period,t in Time[p]}:
EV_E_stored_plug_in[u,p,t] >= EV_limit_di*EV_capacity*n_vehicles*Units_Use[u]* (1-EV_plugged_out[p,t]);
	
	
subject to EV_c5c{u in UnitsOfType['EV'],p in Period,t in Time[p]}:
EV_E_stored_plug_out[u,p,t] <= (EV_limit_ch*EV_capacity + EV_displacement_init[p])*Units_Use[u] * n_vehicles * EV_plugged_out[p,t];

subject to EV_c6c{u in UnitsOfType['EV'],p in Period,t in Time[p]}:
EV_E_stored_plug_out[u,p,t] >= (EV_limit_di*EV_capacity + EV_displacement_init[p])*Units_Use[u] * n_vehicles * EV_plugged_out[p,t];
																			#-
